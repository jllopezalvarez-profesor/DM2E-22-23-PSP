...
public class CajaGlobos {
	...
	public synchronized void aniadir(Globo globo) throws InterruptedException {
		...
		System.out.printf("Un hinchador está intentando añadir un globo %s a la caja. Antes de añadir, en la caja tenemos: %s.\n", globo, globos);
		
		// Esperamos hasta que haya espacio en la cola
		while (globos.size() >= tamanioMaximoCola) {
			wait();
		}
	...
	public synchronized Globo coger() throws InterruptedException {
		// Traza con los globos en la caja
		System.out.printf("Un pinchador está intentando tomar un globo de la caja. Antes de cogerlo, en la caja tenemos: %s.\n", globos);
